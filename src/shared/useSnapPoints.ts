...
export function useSnapPoints({
  ...
  const shouldFade = computed(
    () =>
      (snapPoints.value &&
        ...
        snapPoints.value[fadeFromIndex?.value ?? -1] ===
          activeSnapPoint.value) ||
      !snapPoints.value
  );
  ...
  const snapPointsOffset = computed(
    () =>
      snapPoints.value?.map((snapPoint) => {
        ...
        if (isPx) snapPointAsNumber = Number.parseInt(snapPoint, 10);

        if (isVertical(direction.value)) {
          const height = isPx
            ? snapPointAsNumber
            : windowDimensions.value
              ? snapPoint * windowDimensions.value.innerHeight
              : 0;
        ...
        const width = isPx
          ? snapPointAsNumber
          : windowDimensions.value
            ? snapPoint * windowDimensions.value.innerWidth
            : 0;
  ...
  function onRelease({
    ...
    if (fadeFromIndex.value === undefined) return;
    ...
    const isOverlaySnapPoint =
      activeSnapPointIndex.value === fadeFromIndex.value - 1;
    ...
    if (
      ...
      if (dragDirection > 0 && isLastSnapPoint) {
        snapToPoint(
          snapPointsOffset.value[(snapPoints.value?.length ?? 0) - 1]
        );
        return;
      }
      ...
      snapToPoint(
        snapPointsOffset.value[activeSnapPointIndex.value + dragDirection]
      );
      return;
    }
  ...
  function getPercentageDragged(
    ...
    const isOverlaySnapPointOrHigher =
      activeSnapPointIndex.value >= fadeFromIndex.value;
