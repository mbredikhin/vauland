...
export function useDrawer(
  props: UseDrawerProps & DialogEmitHandlers
): DrawerRootContext {
  const {
    emitDrag,
    emitRelease,
  ...
  function shouldDrag(el: EventTarget | null, isDraggingInDirection: boolean) {
    ...
    const swipeAmount = drawerRef.value
      ? getTranslate(drawerRef.value.$el, direction.value)
      : null;
    const date = new Date();
    ...
    if (
      ...
      date.getTime() - lastTimeDragPrevented.value.getTime() <
        scrollLockTimeout.value &&
      swipeAmount === 0
    ) {
  ...
  function onPress(event: PointerEvent) {
    ...
    pointerStart.value = isVertical(direction.value)
      ? event.clientY
      : event.clientX;
  }
  ...
  function onDrag(event: PointerEvent) {
    ...
    if (isDragging.value) {
      ...
      const draggedDistance =
        (pointerStart.value -
          (isVertical(direction.value) ? event.clientY : event.clientX)) *
        directionMultiplier;
  ...
  function closeDrawer(fromWithin?: boolean) {
    ...
    emitClose();
    if (!fromWithin) isOpen.value = false;
  ...
  function onRelease(event: PointerEvent) {
    ...
    const timeTaken =
      dragEndTime.value.getTime() - dragStartTime.value.getTime();
    const distMoved =
      pointerStart.value -
      (isVertical(direction.value) ? event.clientY : event.clientX);
  ...
  function onNestedDrag(percentageDragged: number) {
    ...
    const initialDim = isVertical(direction.value)
      ? window.innerHeight
      : window.innerWidth;
